...
class SecretShare:
    ...
    def lagrange(self, lst) -> list[int]:
        def norm_sum(j, i):
            return (j * pow(j - i, self.q - 2, self.q)) if i != j else 1
    ...
    def share_poly(self, pol) -> list[SecretSharePoly]:
        """
        Creates a share of polynomials for each participant.
        """
        shares = [self.__share(i) for i in self.polynomial.pol_to_arr(pol)]
